...
// Snapshot satisfies bench.LiveEngine for live renderers. Bandwidth engines
// surface the running EWMA/StdDev/CV plus byte and sample tallies.
func (e *benchEngine) Snapshot() bench.Snapshot {
	...
	return bench.Snapshot{
		...
		StdDev:    stddev,
		CV:        cv,
		Bytes:     e.engine.GetBytesDownloaded(),
...
func packResult(cfg Config, started time.Time, res *Result) bench.Result {
	r := bench.NewResult(bench.KindDownload, cfg.URL, started, 0)
	if res == nil {
		return r
	}
