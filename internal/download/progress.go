...
func (lp *LiveProgress) RenderFrame(mean, ewma, stddev, cv float64, threads, samples int, converged bool, bytesDownloaded int64) string {
	...
	output.WriteString(cyan("║") + " DOWNLOAD TEST IN PROGRESS                           " + cyan("║") + "\n")
...
func (lp *LiveProgress) RenderSummary(result *Result) string {
	...
	output.WriteString(fmt.Sprintf("%s ║ DOWNLOAD TEST COMPLETE%-34s║\n", cyan(""), " "))
	...
	output.WriteString(fmt.Sprintf("%s ║ Bytes Downloaded: %s%-30s%s│\n", cyan(""),
		"", format.Bytes(result.BytesDownloaded), cyan("")))
